...
pbxProject.prototype.removeProjectReference = function (uuid) {
    var project = this.getRootProject();
    if (project.projectReferences) {
        for (var i = 0; i < project.projectReferences.length; i++) {
            var reference = project.projectReferences[i];
            if (reference.ProjectRef === uuid) {
                this.removeFromPbxFileReferenceSectionById(uuid);
                this.removeFromPbxContainerItemProxySection(uuid);
                var productGroup = reference.ProductGroup;
                project.projectReferences.splice(i, 1);
                var group =  this.pbxGroupSection()[productGroup];
                group.uuid = productGroup;
                if (group) {
...
pbxProject.prototype.removeProduct = function (uuid) {
    delete this.pbxGroupSection()[uuid]

}
pbxProject.prototype.removeGroupByName = function (name) {
    var groups = this.pbxGroupSection(),
        key, groupKey, group;
...
pbxProject.prototype.removeGroup = function (group) {
   delete this.pbxGroupSection()[group.uuid]
    var groupChildren = group.children;
    for (var i = 0; i < groupChildren.length; i++) {
        var child = groupChildren[i];
        var fileUuid = child.value;
        var file = this.pbxFileReferenceSection()[fileUuid];
        if (!file) {
            file = this.getPbxBuildFileByFileRef(fileUuid);
            if (file) {
                this.removeFromPbxReferenceProxySection(file);
                fileUuid = file.uuid;
            }
        }
        else {
        ...
        if (file) {
            if (file.isa === 'PBXGroup') {
                this.removeGroup(file.name);
            }
            else {
                if (/wrapper\.pb-project/.test(file.lastKnownFileType)) {
                    this.removeProjectReference(fileUuid);
                }
                this.removeFromPbxBuildFileSectionById(fileUuid);
                this.removeFromPbxFileReferenceSectionById(fileUuid);
                this.removeFromPbxFrameworksBuildPhaseById(fileUuid);    // PBXFrameworksBuildPhase
                // this.removeFromPbxContainerItemProxySection(file);
            }
        }
    }
}
...
pbxProject.prototype.removeFromPbxBuildFileSection = function(file) {
    ...
    for (uuid in this.pbxBuildFileSection()) {
        if (this.pbxBuildFileSection()[uuid].fileRef_comment == file.basename) {
            file.uuid = uuid;
            removeFromPbxBuildFileSectionById(uuid);
            return file;
        }
    }
}

pbxProject.prototype.removeFromPbxBuildFileSectionById = function (uuid) {
...
pbxProject.prototype.removeFromPbxFileReferenceSection = function(file) {
    ...
    for (i in this.pbxFileReferenceSection()) {
        if (this.pbxFileReferenceSection()[i].name == refObj.name ||
            ...
            this.removeFromPbxFileReferenceSectionById(i);
            return file;
        }
    }
    return file;
...
pbxProject.prototype.getPbxFileReferenceByFileRef = function (fileRef) {

    var i;
    for(i in this.pbxFileReferenceSection()) {
        if(this.pbxFileReferenceSection()[i].fileRef == fileRef) {
            file.uuid = i;
            return file;
        }
    }
...
pbxProject.prototype.getPbxBuildFileByFileRef = function (fileRef) {

    var i;
    for(i in this.pbxBuildFileSection()) {
        if(this.pbxBuildFileSection()[i].fileRef == fileRef) {
            var file = this.pbxBuildFileSection()[i];
            file.uuid = i;
            return file;
        }
    }
...
pbxProject.prototype.removeFromPbxFileReferenceSectionById = function(uuid) {
    delete this.pbxFileReferenceSection()[uuid];
    var commentKey = f("%s_comment", uuid);
...
pbxProject.prototype.removeFromPbxReferenceProxySection = function (file) {
    var uuid;

    for(uuid in this.pbxReferenceProxySection()) {
        if(uuid === file.fileRef) {
            this.removeFromPbxReferenceProxySectionById(uuid);
        }
    }
}

pbxProject.prototype.removeFromPbxReferenceProxySectionById = function (uuid) {
    var ref = this.pbxReferenceProxySection()[uuid];
    this.removeFromPbxContainerItemProxySection(ref.remoteRef);
    delete this.pbxReferenceProxySection()[uuid];
    var commentKey = f("%s_comment", uuid);
    delete this.pbxReferenceProxySection()[commentKey];
}


pbxProject.prototype.removeFromPbxContainerItemProxySection = function (uuid) {
    var i;

    for(i in this.pbxContainerItemProxySection()) {
        if(this.pbxContainerItemProxySection()[i].containerPortal == uuid ||
            i === uuid) {
            this.removeFromPbxContainerItemProxySectionById(i);
        }
    }
}

pbxProject.prototype.removeFromPbxContainerItemProxySectionById = function (uuid) {
    delete this.pbxContainerItemProxySection()[uuid];
    var commentKey = f("%s_comment", uuid);
    delete this.pbxContainerItemProxySection()[commentKey];
    this.removeFromPbxTargetDependencySection(uuid);
}

pbxProject.prototype.removeFromPbxTargetDependencySectionById = function (uuid) {
    delete this.pbxTargetDependencySection()[uuid];
    var commentKey = f("%s_comment", uuid);
    delete this.pbxTargetDependencySection()[commentKey];
}

pbxProject.prototype.removeFromPbxTargetDependencySection = function (targetproxy) {
    var i;

    for(i in this.pbxTargetDependencySection()) {
        if(this.pbxTargetDependencySection()[i].targetProxy == targetproxy ||
            i === targetproxy) {
            this.removeFromPbxTargetDependencySectionById(i);
        }
    }
}
...
pbxProject.prototype.removeFromPbxGroup = function(name, file) {
    var pluginsGroupChildren = this.pbxGroupByName(name).children, i;
...
pbxProject.prototype.removeFromPbxGroupByValue = function (name, value) {
    var pluginsGroupChildren = this.pbxGroupByName(name).children, i;
    for(i in pluginsGroupChildren) {
        if(value == pluginsGroupChildren[i].value) {
            pluginsGroupChildren.splice(i, 1);
            break;
        }
    }
...
pbxProject.prototype.removeFromPluginsPbxGroup = function (file) {
    this.removeFromPbxGroup('Plugins', file);
}
...
pbxProject.prototype.removeFromResourcesPbxGroup = function(file) {
    this.removeFromPbxGroup('Resources', file);
}
...
pbxProject.prototype.removeFromFrameworksPbxGroup = function(file) {
    this.removeFromPbxGroup('Frameworks', file);
}
...
pbxProject.prototype.removeFromPbxFrameworksBuildPhaseById = function (uuid) {
    var sources = this.pbxFrameworksBuildPhaseObj();
    for(i in sources.files) {
        if(sources.files[i].value == uuid) {
            sources.files.splice(i, 1);
            break;
        }
    }
...
pbxProject.prototype.pbxContainerItemProxySection = function () {
   
    return this.hash.project.objects['PBXContainerItemProxy'];
}

pbxProject.prototype.pbxReferenceProxySection = function () {
   
    return this.hash.project.objects['PBXReferenceProxy'];
}

pbxProject.prototype.pbxTargetDependencySection = function () {
   
    return this.hash.project.objects['PBXTargetDependency'];
}

pbxProject.prototype.pbxGroupSection= function () {
    return this.hash.project.objects['PBXGroup'];
}
...
pbxProject.prototype.getRootProject = function () {
    return this.pbxProjectSection()[this.hash.project.rootObject];
}
